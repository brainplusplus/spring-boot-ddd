...
function editData(id){
	...
	$("#divFormData").load("/admin/master/master_laporan_mingguan_tampak_images/form/"+varId,function(data){
		$('#laporanId').val(varId);
		$("#formData").attr('action','/rest/master/master_laporan_mingguan_tampak_images/update');
...
function confirmDeleteAction(id){
	$.get("/rest/master/master_laporan_mingguan_tampak_images/remove/"+id, function (result) {
...
function gotoNew(){
	...
	$("#divFormData").load("/admin/master/master_laporan_mingguan_tampak_images/form/"+varId,function(data){
		$('#laporanId').val(varId);
		$("#formData").attr('action','/rest/master/master_laporan_mingguan_tampak_images/save');
		hapusGbr();
		$('#divListData').fadeOut('fast', function(){
        	$('#divFormData').fadeIn('fast');
    	});
	...
	table = $('#grid').DataTable({
	    ...
	    "bSort" : false,
	     ajax: function(data,callback, settings) {
	        ...
	        $.get('/rest/master/master_laporan_mingguan_tampak_images/search/'+varId,{search:nama,rows: data.length,page: page + 1},function(res) {
