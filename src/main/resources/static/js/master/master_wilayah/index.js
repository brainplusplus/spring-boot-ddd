...
function editData(id) {
    $("#divFormData").load("/admin/master/master_wilayah/form", function (data) {
        $("#formData").attr('action', '/rest/master/master_wilayah/update');
...
function confirmDeleteAction(id) {
    $.get("/rest/master/master_wilayah/remove/" + id, function (result) {
...
function gotoNew() {
    $("#divFormData").load("/admin/master/master_wilayah/form", function (data) {
        $("#formData").attr('action', '/rest/master/master_wilayah/save');
        $('#divListData').fadeOut('fast', function () {
            $('#divFormData').fadeIn('fast');
        });
    });
...
function initGrid() {
    table = $('#grid').DataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.get('/rest/master/master_wilayah/search', {
                search: nama,
                rows: data.length,
                page: page + 1
            }, function (res) {
