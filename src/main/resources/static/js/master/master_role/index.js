...
function editData(id) {
    $("#divFormData").load("/admin/master/master_role/form", function (data) {
                $("#formData").attr('action', '/rest/master/master_role/update');
...
function detailPage(id) {
    location.href = "/admin/master/master_role_page/index/" + id;
}
...
function confirmDeleteAction(id) {
    $.get("/rest/master/master_role/remove/" + id, function (result) {
...
//	});
    $("#divFormData").load("/admin/master/master_role/form", function (data) {
    			$("#formData").attr('action', '/rest/master/master_role/save');
                $('#divListData').fadeOut('fast', function () {
                    $('#divFormData').fadeIn('fast');
                });
//				
...
function initGrid() {
    table = $('#grid').DataTable({
        ...
        ajax: function (data, callback, settings) {
            ...
            $.get('/rest/master/master_role/search', {search: nama, rows: data.length, page: page + 1}, function (res) {
        ...
        "columns": [
            {
                ...
                "render": function (data, type, row) {
                    return "<a href='#' class='btn btn-default' onclick=\"editData('" + data + "');\"><span title='Ubah' class='glyphicon glyphicon-edit'></span></a> " +
                        ...
                        " <a href='#' class='btn btn-info' onclick=\"detailPage('" + data + "');\" >Detail Page</a>" ;
                }
            },
            {data: "id"},
            {data: "nama"}
