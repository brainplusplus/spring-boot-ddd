...
function editData(id){
	$("#divFormData").load("/admin/master/master_kecamatan/form",function(data){
			$("#formData").attr('action','/rest/master/master_kecamatan/update');
...
function confirmDeleteAction(id){
	$.get("/rest/master/master_kecamatan/remove/"+id, function (result) {
...
function gotoNew(){
	$("#divFormData").load("/admin/master/master_kecamatan/form",function(data){
			$("#formData").attr('action','/rest/master/master_kecamatan/save');
			$('#divListData').fadeOut('fast', function(){
	        	$('#divFormData').fadeIn('fast');
	    	});
	});
	...
	table = $('#grid').DataTable({
	    ...
	    "bSort" : false,
	     ajax: function(data,callback, settings) {
	        ...
	        $.get('/rest/master/master_kecamatan/search',{search:nama,rows: data.length,page: page + 1},function(res) {
			...
			{ 
				...
				"render": function ( data, type, row ) 
							{ 
								...
								"|| <a href='#' onclick='confirmDelete(\""+data+"\");'>Delete</a>";
							}  
			},        
	        {data: "kode"},
	        {data: "nama"},
